...
# It then call /colesfs/java/bin/jstat -gc and /colesfs/java/bin/jstat -gccapacity to catch current and
...
function usage() {
    local prog="${1:-check_/colesfs/java/bin/jstat.sh}"
    echo "Usage: $prog -v";
    echo "       Print version and exit"
    echo "Usage: $prog -h";
    echo "      Print this help and exit"
    echo "Usage: $prog -p <pid> [-w <%ratio>] [-c <%ratio>]";
    echo "Usage: $prog -f <pidfile> [-w <%ratio>] [-c <%ratio>]";
    ...
    echo "       -f <file>      the PID file of process to monitor"
...
while getopts hvpf:s:j:w:c: opt ; do
    case ${opt} in
...
if [ -z "$pid" -a -z "$service" -a $use_jps -eq 0 ] ; then
    echo "One of -f, -p, -s or -j parameter must be provided"
    usage $0
    exit 3
fi
...
gc=$(/colesfs/java/bin/jstat -gc $pid | tail -1 | sed -e 's/[ ][ ]*/ /g')
if [ -z "$gc" ]; then
...
gccapacity=$(/colesfs/java/bin/jstat -gccapacity $pid | tail -1 | sed -e 's/[ ][ ]*/ /g')
...
if [ $cs -gt 0 -a $permratio -ge $cs ]; then
    echo "CRITICAL: /colesfs/java/bin/jstat process $label critical PermGen (${permratio}% of MaxPermSize)|$perfdata"
    exit 2
fi
if [ $cs -gt 0 -a $heapratio -ge $cs ]; then
    echo "CRITICAL: /colesfs/java/bin/jstat process $label critical Heap (${heapratio}% of MaxHeapSize)|$perfdata"
    exit 2
fi

if [ $ws -gt 0 -a $permratio -ge $ws ]; then
    echo "WARNING: /colesfs/java/bin/jstat process $label warning PermGen (${permratio}% of MaxPermSize)|$perfdata"
    exit 1
fi
if [ $ws -gt 0 -a $heapratio -ge $ws ]; then
    echo "WARNING: /colesfs/java/bin/jstat process $label warning Heap (${heapratio}% of MaxHeapSize)|$perfdata"
    exit 1
fi
echo "OK: /colesfs/java/bin/jstat process $label alive|$perfdata"
exit 0
